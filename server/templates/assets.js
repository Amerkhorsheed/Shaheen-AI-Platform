...
/**
 * Brand assets used by the printable document templates.
 *
 * Inlined rather than linked so a document keeps its emblem when it is saved,
 * printed, or opened from a machine with no network.
 */

// High-definition Syrian golden eagle with three stars.
...
/** Re-render the emblem at a different size. */
function eagleEmblem({ width = 80, height = 60 } = {}) {
  return SYRIAN_EAGLE_SVG.replace('width="80" height="60"', `width="${width}" height="${height}"`);
}

module.exports = { SYRIAN_EAGLE_SVG, eagleEmblem };
